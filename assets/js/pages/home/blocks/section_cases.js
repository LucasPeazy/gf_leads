...
export function casesToSlider() {
    const mainBlock = document.querySelector('.case_slider'); // Основной блок
    const wrapperBlock = document.querySelector('.case_slider .row'); // Внутренний контейнер (для wrapper)
    const sliderItems = document.querySelectorAll('.case_slider .row .col-md-6, .case_slider .row .col-xl-4'); // Элементы слайда
    ...
    function initSwiper() {
        if (window.innerWidth <= 767) {
            if (!caseSwiper) {
                ...
                caseSwiper = new Swiper('.case_slider', {
                    slidesPerView: 1,
                    spaceBetween: 15,
                    loop: true,
                    autoHeight: true,
